...
getSysParam;
Tstart = 1;
TusedAll = [30 50 60:20:140];
ngAll = 100;%All = fliplr([20 50 100 200]);
dT = 5; %All = [5 10 20 25 50]%[5 10 20 30 50]% 100]; % [5 10 20 30 50 75]; % [5 10 20 30 50 100];
...
meanNormAbsErrHeatMap_MPL = zeros(length(ngAll), length(TusedAll));
meanNormAbsErrHeatMap_cluster = zeros(length(ngAll), length(TusedAll));

medianNormAbsErrHeatMap_MPL = zeros(length(ngAll), length(TusedAll));
medianNormAbsErrHeatMap_cluster = zeros(length(ngAll), length(TusedAll));


meanAUROC_ben_HeatMap_MPL = zeros(length(ngAll), length(TusedAll));
meanAUROC_ben_HeatMap_Epi_si = zeros(length(ngAll), length(TusedAll));
meanAUROC_ben_HeatMap_Epi_sij = zeros(length(ngAll), length(TusedAll));
meanAUROC_ben_HeatMap_EpiWithR_si = zeros(length(ngAll), length(TusedAll));
meanAUROC_ben_HeatMap_EpiWithR_sij = zeros(length(ngAll), length(TusedAll));

meanAUROC_del_HeatMap_MPL = zeros(length(ngAll), length(TusedAll));
meanAUROC_del_HeatMap_Epi_si = zeros(length(ngAll), length(TusedAll));
meanAUROC_del_HeatMap_Epi_sij = zeros(length(ngAll), length(TusedAll));
meanAUROC_del_HeatMap_EpiWithR_si = zeros(length(ngAll), length(TusedAll));
meanAUROC_del_HeatMap_EpiWithR_sij = zeros(length(ngAll), length(TusedAll));

meanAUROCSelc_ben_HeatMap_MPL = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_ben_HeatMap_Epi_si = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_ben_HeatMap_Epi_sij = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_ben_HeatMap_EpiWithR_si = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_ben_HeatMap_EpiWithR_sij = zeros(length(ngAll), length(TusedAll));

meanAUROCSelc_del_HeatMap_MPL = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_del_HeatMap_Epi_si = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_del_HeatMap_Epi_sij = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_del_HeatMap_EpiWithR_si = zeros(length(ngAll), length(TusedAll));
meanAUROCSelc_del_HeatMap_EpiWithR_sij = zeros(length(ngAll), length(TusedAll));

for nn = 1:length(ngAll)
    ng = ngAll(nn);
    for dd = 1:length(TusedAll)
        Tused = TusedAll(dd);
        Tend = Tused + Tstart;
        numTps = Tused/dT;
...
%%
if(saveFile == 1)
    fileNameSave = ['FiveSites_Set' num2str(thisSet) '_numStrains' num2str(numStrainsInInitialPop) '_HeatmapTused_ng' num2str(ng) '_dT' num2str(dT) '_Tused' num2str(TusedAll(1)) '_' num2str(TusedAll(end)) '_reg1' regStr1  '_reg2' regStr2 '_numItr' num2str(numItr) '.mat'];
