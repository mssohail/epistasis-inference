...
%thisSetAll = [218751001 418751001 618851001];
thisSetAll = [218851001 418851001 618851001];
...
numSelcSitesItr_ToPlot = NaN*ones(length(repAll), length(thisSetAll));
numSelcSitesEpiItr_si_ToPlot = NaN*ones(length(repAll), length(thisSetAll));
numSelcSitesEpiItr_sij_ToPlot = NaN*ones(length(repAll), length(thisSetAll));
SE_numSelcSitesEpiItr = NaN*ones(length(repAll), length(thisSetAll));
SE_numSelcSitesEpiItr_si = NaN*ones(length(repAll), length(thisSetAll));
SE_numSelcSitesEpiItr_sij = NaN*ones(length(repAll), length(thisSetAll));
SE_EpiWithR_only_si_SelcBen = NaN*ones(length(repAll), length(thisSetAll));
SE_EpiWithR_only_sij_SelcBen = NaN*ones(length(repAll), length(thisSetAll));
SE_only_si_SelcBen = NaN*ones(length(repAll), length(thisSetAll));

SE_EpiWithR_only_si_SelcDel = NaN*ones(length(repAll), length(thisSetAll));
SE_EpiWithR_only_sij_SelcDel = NaN*ones(length(repAll), length(thisSetAll));
SE_only_si_SelcDel = NaN*ones(length(repAll), length(thisSetAll));
 
mean_aucLinkEpiWithR_si_SelecBen = NaN*ones(length(repAll), length(thisSetAll));
mean_aucLinkEpiWithR_si_SelecDel = NaN*ones(length(repAll), length(thisSetAll));
mean_aucLinkEpiWithR_sij_SelecBen = NaN*ones(length(repAll), length(thisSetAll));
mean_aucLinkEpiWithR_sij_SelecDel = NaN*ones(length(repAll), length(thisSetAll));

mean_aucLink_si_SelecBen = NaN*ones(length(repAll), length(thisSetAll));
mean_aucLink_si_SelecDel = NaN*ones(length(repAll), length(thisSetAll));
...
for i = 1:length(repAll)
    ...
    for j = 1:length(thisSetAll)
        ...
        if(thisSet == 618851001 && thisRep == 25)
            ...
            totalEpiTermsAll(j) = totalEpiTerms;
            LinAll(j) = Lin;
            step = 0.002;
...
%%
save(['D:\New\MPL Epi\Figures\Sites20\PosDelSel\Sparsity_rep_10_30_sites_X18851001.mat'])
...
widthFree = (xDim - leftMargin - rightMargin - 2*hgap1);
...
for ind1 = 4:6
    ha(ind1) = axes('Units','centimeters', ...
                    'Position',[leftMargin+(ind1 - 4)*(width1+hgap1) bottomMargin+height+vgap1 width1 height], ...
                    'XTickLabel','', ...
                    'YTickLabel','');
end
...
for ind1 = 7:9
    ha(ind1) = axes('Units','centimeters', ...
                    'Position',[leftMargin+(ind1 - 7)*(width1+hgap1) bottomMargin width1 height], ...
                    'XTickLabel','', ...
                    'YTickLabel','');
end
...
for i = [1:3]
    ...
    plot(numSelcSitesEpiItr_si_ToPlot(:,i)/LinAll(i), '.-', 'color', color_scheme41(80,:), 'MarkerFaceColor', color_scheme41(80,:), 'lineWidth', 1)
    hold on
    errorbar(numSelcSitesEpiItr_si_ToPlot(:,i)/LinAll(i), SE_numSelcSitesEpiItr_si(:,i), 'k', 'LineStyle', 'none', 'CapSize', 3)
    plot(numSelcSitesEpiItr_sij_ToPlot(:,i)/totalEpiTermsAll(i), '--', 'color', color_scheme41(80,:), 'MarkerFaceColor', color_scheme41(80,:), 'lineWidth', 1)
    errorbar(numSelcSitesEpiItr_sij_ToPlot(:,i)/totalEpiTermsAll(i), SE_numSelcSitesEpiItr_sij(:,i), 'k', 'LineStyle', 'none', 'CapSize', 3)
    ...
    title([num2str(LinAll(i)) '-locus system'])
    axis([0.5 4.5 -0.06 1])
end

ylabel({'Fraction of accessible', 'fitness parameters'})
ylabh2 = get(gca,'ylabel'); 
set(ylabh2,'Units','centimeters');
set(ylabh2,'position',get(ylabh2,'position') + [-(2*width1+1*hgap1+0.75) 0 0]);
...
if(saveFigs == 1)
    figname = [figureDir 'FigureS9_10_30_Sites_numAccParam'];
