...
numStrainsInInitialPop = 5;
repsAll = [5 3 1];%[20 15 10];
...
dirNameFigures = [dirNameFigures 'Sites' num2str(Lin) chosenSlash selTypeName chosenSlash 'Set' num2str(thisSet) chosenSlash 'Rep' chosenSlash];
...
for jj = 1:length(repsAll)
    ...
    fileName = ['PlotData_Rep' num2str(thisRep) '_WFsimEpi_Nmu' str1 '_N1000_L' num2str(Lin) '_D' num2str(DAll) '_Dele' num2str(DeleAll) '_selVal' num2str(selVal) '_delSelVal' num2str(delSelVal) '_initStr' num2str(numStrainsInInitialPop) '_2k_' num2str(numItrIn) '_Tend101_dts10_ng100_linkDiff' '_reg1' regStrIn1  '_reg2' regStrIn2 '_Set' num2str(thisSet) '_itr1_'  num2str(numItrIn-thisRep+1) '_' num2str(numItrIn) '.mat'];
    if(thisRep ~= 1)
        load([dirNameFigures fileName], 'allAuc', 'allAuc_rep', ...
            'absErr_LinkEpi_clusterOnlyAmb_itr', 'absErr_LinkEpi_cluster_itr', 'color_scheme_npg', 'perSiteSelctionEpiRd', ...
            'numSelcSitesEpiItr_si', 'numSelcSitesEpiItr_si_rep', 'estSiteSelctionEpiRd', 'estSiteSelctionEpiRd_rep', ...
            'myColorMap2', 'myLabel', 'mapBlues', 'numSelcSitesEpiItr_sij_rep', 'numSelcSitesEpiItr_sij', ...
                    'Lin', 'allSelcSitesEpi', 'allEstEpiItr', 'allEstEpiItr_rep', ...
                    'aucLinkEpi_only_si_Itr_rep', 'aucLinkEpi_only_si_SelcItr_rep', ...
                    'aucLinkEpi_only_sij_Itr_rep', 'aucLinkEpi_only_sij_SelcItr_rep', ...
                    'aucLinkEpi_only_si_Itr', 'aucLinkEpi_only_si_SelcItr', ...
                    'aucLinkEpi_only_sij_Itr', 'aucLinkEpi_only_sij_SelcItr');
    
         allEstEpiItr_rep_Cell{jj} = allEstEpiItr_rep;
         allEstEpiItr_rep_Cell{3} = allEstEpiItr;

         thisGroup = 1;
         SE_Epi_only_si_Ben_rep(4-jj) = std((aucLinkEpi_only_si_Itr_rep(aucLinkEpi_only_si_Itr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_Itr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_sij_Ben_rep(4-jj) = std((aucLinkEpi_only_sij_Itr_rep(aucLinkEpi_only_sij_Itr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_Itr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_si_SelcBen_rep(4-jj) = std((aucLinkEpi_only_si_SelcItr_rep(aucLinkEpi_only_si_SelcItr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_SelcItr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_sij_SelcBen_rep(4-jj) = std((aucLinkEpi_only_sij_SelcItr_rep(aucLinkEpi_only_sij_SelcItr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_SelcItr_rep(:,thisGroup) ~= -1))*1.96;
         thisGroup = 2;
         SE_Epi_only_si_Del_rep(4-jj) = std((aucLinkEpi_only_si_Itr_rep(aucLinkEpi_only_si_Itr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_Itr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_sij_Del_rep(4-jj) = std((aucLinkEpi_only_sij_Itr_rep(aucLinkEpi_only_sij_Itr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_Itr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_si_SelcDel_rep(4-jj) = std((aucLinkEpi_only_si_SelcItr_rep(aucLinkEpi_only_si_SelcItr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_SelcItr_rep(:,thisGroup) ~= -1))*1.96;
         SE_Epi_only_sij_SelcDel_rep(4-jj) = std((aucLinkEpi_only_sij_SelcItr_rep(aucLinkEpi_only_sij_SelcItr_rep(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_SelcItr_rep(:,thisGroup) ~= -1))*1.96;
    end
    if(thisRep == 1)
       allAuc_rep = allAuc; 
       estSiteSelctionEpiRd_rep = estSiteSelctionEpiRd;
       numSelcSitesEpiItr_si_rep = numSelcSitesEpiItr_si;
       
       SE_Epi_only_si_Ben_rep(4-jj) = std((aucLinkEpi_only_si_Itr(aucLinkEpi_only_si_Itr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_Itr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_sij_Ben_rep(4-jj) = std((aucLinkEpi_only_sij_Itr(aucLinkEpi_only_sij_Itr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_Itr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_si_SelcBen_rep(4-jj) = std((aucLinkEpi_only_si_SelcItr(aucLinkEpi_only_si_SelcItr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_SelcItr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_sij_SelcBen_rep(4-jj) = std((aucLinkEpi_only_sij_SelcItr(aucLinkEpi_only_sij_SelcItr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_SelcItr(:,thisGroup) ~= -1))*1.96;
       thisGroup = 2;
       SE_Epi_only_si_Del_rep(4-jj) = std((aucLinkEpi_only_si_Itr(aucLinkEpi_only_si_Itr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_Itr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_sij_Del_rep(4-jj) = std((aucLinkEpi_only_sij_Itr(aucLinkEpi_only_sij_Itr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_Itr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_si_SelcDel_rep(4-jj) = std((aucLinkEpi_only_si_SelcItr(aucLinkEpi_only_si_SelcItr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_si_SelcItr(:,thisGroup) ~= -1))*1.96;
       SE_Epi_only_sij_SelcDel_rep(4-jj) = std((aucLinkEpi_only_sij_SelcItr(aucLinkEpi_only_sij_SelcItr(:,thisGroup) ~= -1,thisGroup)))/sqrt(sum(aucLinkEpi_only_sij_SelcItr(:,thisGroup) ~= -1))*1.96;
    end
    
    
   if(jj == 1)
        numSelcSitesEpiItr_sij_rep1 = numSelcSitesEpiItr_sij_rep;
        numRep1 = thisRep;
    elseif(jj == 2)
        numSelcSitesEpiItr_sij_rep2 = numSelcSitesEpiItr_sij_rep;
        numRep2 = thisRep;
    elseif(jj == 3)
        numSelcSitesEpiItr_sij_rep3 = numSelcSitesEpiItr_sij;
        numRep3 = thisRep;
    end
...
%    aucLinkEpi_only_si_Itr(:,1)
    meanAuc_si_sij_rep(4-jj,:) = [allAuc_rep([5 6], 1)' allAuc_rep([5 6], 2)']
    meanAucSelc_si_sij_rep(4-jj,:) = [allAuc_rep([5 6], 3)' allAuc_rep([5 6], 4)']
    
    meanAuc_si_MPL_SL(4-jj,:) = [allAuc_rep([3 4], 1)' allAuc_rep([3 4], 2)']
    meanAucSelc_si_MPL_SL(4-jj,:) = [allAuc_rep([3 4], 3)' allAuc_rep([3 4], 4)']
    ...
    % plot estimate FL
    axes(ha(jj+6))
    circularGraph_noButtons_colorGradient(estSiteSelctionEpiRd_rep,'Colormap',myColorMapGradient,'Label',myLabel);
    ...
    if(jj == 2)
        xlabel('Fraction of accessible coefficients')
    end
    %xlabh = get(gca,'xlabel'); 
    %set(xlabh,'Units','normalized');
    %set(xlabh,'position',get(xlabh,'position') + [-0.15 -0.1 0]);
    axis([0 1.2 0 1])
    %title(['\fontsize{7.5}' num2str(thisRep) ' rep.'])
    %title(['\fontsize{7.5} Replicates: ' num2str(thisRep)])
    if(thisRep == 1)
        title([num2str(thisRep) '\fontsize{7.5} replicate'])
    else
        title([num2str(thisRep) '\fontsize{7.5} replicates'])
    end
...
%     thisRep = repsAll(jj);
%     
%     fileName = ['PlotData_WFsimEpi_Nmu' str1 '_N1000_L5_D2_Dele2_selVal50_delSelVal-50_initStr' num2str(numStrainsInInitialPop) '_2k_1000_Tend301_dts10_ng100_linkDiff' '_reg1' regStrIn1  '_reg2' regStrIn2 '_Set' num2str(thisSet) '_itr1_1000.mat'];
%     load([dirNameFigures fileName], 'allAuc_rep');
%     
% %    aucLinkEpi_only_si_Itr(:,1)
%     meanAuc_si_sij_rep(jj,:) = [allAuc_rep([5 6], 1)' allAuc_rep([5 6], 2)']
%     meanAucSelc_si_sij_rep(jj,:) = [allAuc_rep([5 6], 3)' allAuc_rep([5 6], 4)']
%  
%     meanAuc_si_MPL_SL(jj,:) = [allAuc_rep([3 4], 1)' allAuc_rep([3 4], 2)']
%     meanAucSelc_si_MPL_SL(jj,:) = [allAuc_rep([3 4], 3)' allAuc_rep([3 4], 4)']
% end


% plot AUROC
%meanAucSelc_si_sij_rep

axes(ha(4))
SE_sc_all_rep = reshape([SE_Epi_only_si_Ben_rep; SE_Epi_only_si_Del_rep], 1, 6);
xCord = [1-.1425 1+.1425 2-.1425 2+.1425 3-.1425 3+.1425];
yCord = reshape([meanAuc_si_sij_rep(:,1) meanAuc_si_sij_rep(:,3)]', 1, 6);

bb = bar([meanAuc_si_sij_rep(:,1) meanAuc_si_sij_rep(:,3)])
hold on
errorbar(xCord, yCord, SE_sc_all_rep, 'k', 'LineStyle', 'none', 'CapSize', 5)
...
axes(ha(5))
SE_sc_selc_rep = reshape([SE_Epi_only_si_SelcBen_rep; SE_Epi_only_si_SelcDel_rep], 1, 6);
xCord_selc = [1-.1425 1+.1425 2-.1425 2+.1425 3-.1425 3+.1425];
yCord_selc = reshape([meanAucSelc_si_sij_rep(:,1) meanAucSelc_si_sij_rep(:,3)]', 1, 6);

bz = bar([meanAucSelc_si_sij_rep(:,1) meanAucSelc_si_sij_rep(:,3)])
hold on
errorbar(xCord_selc, yCord_selc, SE_sc_selc_rep, 'k', 'LineStyle', 'none', 'CapSize', 5)
...
axes(ha(10))
SE_epi_all_rep = reshape([SE_Epi_only_sij_Ben_rep; SE_Epi_only_sij_Del_rep], 1, 6);
xCord = [1-.1425 1+.1425 2-.1425 2+.1425 3-.1425 3+.1425];
yCord = reshape([meanAuc_si_sij_rep(:,2) meanAuc_si_sij_rep(:,4)]', 1, 6);

bb = bar([meanAuc_si_sij_rep(:,[2 4])])
hold on
errorbar(xCord, yCord, SE_epi_all_rep, 'k', 'LineStyle', 'none', 'CapSize', 5)
...
axis([0.5 3.5 0.6 1])
ylabel('AUROC')
xlabel('Number of replicates')% in starting population')
...
axes(ha(11))
SE_epi_selc_rep = reshape([SE_Epi_only_sij_SelcBen_rep; SE_Epi_only_sij_SelcDel_rep], 1, 6);
xCord_selc = [1-.1425 1+.1425 2-.1425 2+.1425 3-.1425 3+.1425];
yCord_selc = reshape([meanAucSelc_si_sij_rep(:,2) meanAucSelc_si_sij_rep(:,4)]', 1, 6);
xCord_selc = xCord_selc(3:end);
yCord_selc = yCord_selc(3:end);
SE_epi_selc_rep = SE_epi_selc_rep(3:end);

meanAucSelc_si_sij_rep(1,4) = NaN; % because this value is computed over less than 5 samples (specifically 3 samples)
bz = bar([meanAucSelc_si_sij_rep(:,[2 4])])
hold on
errorbar(xCord_selc, yCord_selc, SE_epi_selc_rep, 'k', 'LineStyle', 'none', 'CapSize', 5)
...
for jj = 1:3
    ...
    axes(ha(11+jj))
    if(jj == 1)
        h1 = histogram(numSelcSitesEpiItr_sij_rep1/10, [0:.2:1.2])
    elseif(jj == 2)
        h1 = histogram(numSelcSitesEpiItr_sij_rep2/10, [0:.2:1.2])
    elseif(jj == 3)
        h1 = histogram(numSelcSitesEpiItr_sij_rep3/10, [0:.2:1.2])
    end
    ...
    if(jj==1)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep1)])
        title([num2str(numRep1) '\fontsize{7.5} replicates'])
    elseif(jj==2)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep2)])
        title([num2str(numRep2) '\fontsize{7.5} replicates'])
    elseif(jj==3)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep3)])
        title([num2str(numRep3) '\fontsize{7.5} replicate'])
    end
...
box1.Units = 'centimeter';
box1.Position = [10 3.4+vgap2+height11+0.05 0.485 0.28];

box2 = annotation('rectangle',dimDummy,'FaceColor',myColorMap3(3,:))
box2.Units = 'centimeter';
box2.Position = [10 3+vgap2+height11+0.05 0.485 0.28];

textLeg1 = annotation('textbox',dimDummy,'String','Beneficial','FitBoxToText','on')
textLeg1.Units = 'centimeter';
textLeg1.Position = [10.4 3.3+vgap2+height11+0.1 2 0.5];
textLeg1.LineStyle = 'none';
...
textLeg2 = annotation('textbox',dimDummy,'String','Deleterious','FitBoxToText','on')
textLeg2.Units = 'centimeter';
textLeg2.Position = [10.4 2.9+vgap2+height11+0.1 2 0.5];
textLeg2.LineStyle = 'none';
...
te1 = annotation('textbox',dimDummy,'String','1 replicate','FitBoxToText','on')
te1.Units = 'centimeter';
te1.Position = [8.2 8.9+4.5 6 0.5];%[5.8 4.1 0.7 0.5];
te1.LineStyle = 'none';
te1.FontWeight = 'bold';
te1.FontName = 'Arial';
te1.FontSize = chosenFontSize;

te2 = annotation('textbox',dimDummy,'String','3 replicates','FitBoxToText','on')
te2.Units = 'centimeter';
te2.Position = [12.4 8.9+4.5 6 0.5];%[5.8 4.1 0.7 0.5];
te2.LineStyle = 'none';
te2.FontWeight = 'bold';
te2.FontName = 'Arial';
te2.FontSize = chosenFontSize;

te3 = annotation('textbox',dimDummy,'String','5 replicates','FitBoxToText','on')
te3.Units = 'centimeter';
te3.Position = [16.58 8.9+4.5 6 0.5];%[5.8 4.1 0.7 0.5];
te3.LineStyle = 'none';
te3.FontWeight = 'bold';
te3.FontName = 'Arial';
...
if(saveFigs == 1)
    figname = [figureDir 'Figure5_NumReps_4'];
...
ha(10) = axes('Units','centimeters', ...
                'Position',[leftMargin bottomMargin+1 width12 height11], ...
                'XTickLabel','', ...
                'YTickLabel','');

ha(11) = axes('Units','centimeters', ...
                'Position',[leftMargin+hgap12+width12 bottomMargin+1 width13 height11], ...
                'XTickLabel','', ...
                'YTickLabel','');
...
for jj = 1:3
    ...
    if(jj==1)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep1)])
        title([num2str(numRep1) '\fontsize{7.5} replicate'])
    elseif(jj==2)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep2)])
        title([num2str(numRep2) '\fontsize{7.5} replicates'])
    elseif(jj==3)
        %title(['\fontsize{7.5} Replicates: ' num2str(numRep3)])
        title([num2str(numRep3) '\fontsize{7.5} replicates'])
    end
...
save('Data_NumRep_onlyEpiTerms_2.mat')
if(saveFigs == 1)
    figname = 'NumRep_onlyEpiTerms_2';
...
axes(ha(15))
estSiteSelctionEpiRd_1 = getMeanFL_estSiteSelctionEpi(Lin, allEstEpiItr_rep_Cell{1});
circularGraph_noButtons(estSiteSelctionEpiRd_1*50,'Colormap',myColorMap3,'Label',myLabel);

axes(ha(16))
estSiteSelctionEpiRd_2 = getMeanFL_estSiteSelctionEpi(Lin, allEstEpiItr_rep_Cell{2});
circularGraph_noButtons(estSiteSelctionEpiRd_2*50,'Colormap',myColorMap3,'Label',myLabel);

axes(ha(17))
estSiteSelctionEpiRd_3 = getMeanFL_estSiteSelctionEpi(Lin, allEstEpiItr_rep_Cell{3});
circularGraph_noButtons(estSiteSelctionEpiRd_3*50,'Colormap',myColorMap3,'Label',myLabel);



dimDummy = [0.1 0.1 0.1 0.1]; % dummy position (in normalized units)
ta = annotation('textbox',dimDummy,'String','Fitness la','FitBoxToText','on')
ta.Units = 'centimeter';
ta.Position = [5 1 3 0.5];
ta.LineStyle = 'none';
ta.FontWeight = 'bold';
ta.FontName = 'Arial';
ta.FontSize = 12;
