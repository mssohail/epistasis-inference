...
if(runAnalysisCode)
    ...
    if(FLAG_Epi == true)
        ...
        fileNameSelEstEpi = ['SelEstEpi_gamma' num2str(priorConstSC) '_' num2str(priorConstEpi) '.txt'];
    end
    ...
    fileNameAccessEstEpi = ['AccessibilityMPLEpi.txt'];
    ...
    if(FLAG_MarkAccessibility && (size(intCovMtx, 1) < 5000))
       ...
       %dlmwrite([dirNameAnalysis 'Estimates' chosenSlash  fileNameAccessEst], selcSites);
       
       % for Epi
       if(FLAG_Epi == true)
           if(size(intCovMtxEpi, 1) < 50000)
               [selcSitesEpi, wellCondSitesEpi, illCondSitesEpi, clusterEpi] = findIndColsOfHmat(intCovMtxEpi);

               if(exist([dirNameAnalysis 'Estimates' chosenSlash], 'dir') == 0)
                    mkdir([dirNameAnalysis 'Estimates' chosenSlash])
               end
               dlmwrite([dirNameAnalysis 'Estimates' chosenSlash  fileNameAccessEstEpi], selcSitesEpi);
           else
...
%        end
    ...
    if(FLAG_SaveFile)
        ...
        if(FLAG_RepComb)
            save([dirNameAnalysis 'Estimates' chosenSlash 'tempDataFileforRepComb.mat'], ...
